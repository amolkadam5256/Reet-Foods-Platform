# Google Business Profile (GBP)

Project

Reet Foods & Gifting

Website

https://www.reetfoodsngiftings.com

---

# Purpose

Google Business Profile helps the business appear in

- Google Search
- Google Maps
- Local Pack
- Knowledge Panel
- Local SEO Results

Primary Objectives

- Improve Local SEO
- Increase Brand Visibility
- Receive Customer Reviews
- Generate Phone Calls
- Increase Website Visits
- Improve Trust Signals

---

# Account Information

| Item          | Value                               |
| ------------- | ----------------------------------- |
| Business Name | Reet Foods & Gifting                 |
| Website       | https://www.reetfoodsngiftings.com |
| Business Type | Online Job Portal                   |
| Status        | Pending                             |
| Verification  | Pending                             |

---

# Step 1

Go to

https://business.google.com/

Sign in with the primary Google account.

Use the same Google account that owns

- Search Console
- Analytics
- Tag Manager

---

# Step 2

Click

Manage Now

or

Add Business

---

# Step 3

Business Name

Enter

Reet Foods & Gifting

Use the exact branding everywhere.

Do not add

- India
- Official
- Pvt Ltd

unless they are part of the legal business name.

---

# Step 4

Business Category

Primary Category

Employment Agency

Alternative Categories

Recruiter

Employment Consultant

Career Guidance Service

Educational Website

Online Career Portal

Recruitment Service

Choose the closest category available in Google.

---

# Step 5

Physical Location

Question

Do customers visit your office?

Recommended

No

Because Reet Foods & Gifting is an online platform.

If you later open a physical office, update this setting.

---

# Step 6

Service Areas

Select

India

If targeting multiple regions, add:

- Maharashtra
- Delhi
- Karnataka
- Tamil Nadu
- Gujarat
- Uttar Pradesh
- Rajasthan
- Telangana
- Kerala
- Punjab

Do not add countries where you don't operate.

---

# Step 7

Contact Details

Phone Number

Use the official business number.

Website

https://www.reetfoodsngiftings.com

Chat

Enable if supported.

---

# Step 8

Verification

Available methods may include:

- Postcard
- Phone
- Email
- Video Verification
- Search Console Verification

Complete verification before optimization.

---

# Step 9

Business Hours

Example

Monday–Friday

09:00 AM – 06:00 PM

Saturday

09:00 AM – 01:00 PM

Sunday

Closed

Update holiday hours when required.

---

# Step 10

Business Description

Maximum

750 characters

Include:

- Government Jobs
- corporate gifting Result
- Latest Notifications
- Admit Cards
- Answer Keys
- Results
- Free Job Alerts
- Central Government Jobs
- State Government Jobs
- PSU Jobs
- Banking Jobs
- Railway Jobs
- Defence Jobs

Avoid keyword stuffing.

---

# Step 11

Business Logo

Recommended

PNG

1024 × 1024

Transparent Background

---

# Step 12

Cover Photo

Recommended

1920 × 1080

Include

- Brand Logo
- Website
- Clean Design

---

# Step 13

Additional Photos

Upload:

- Team
- Office
- Screenshots
- Dashboard
- Website
- Work Environment

Maintain high quality.

---

# Step 14

Services

Examples:

- Government Job Updates
- corporate gifting Result
- Admit Card Alerts
- Free Job Notifications
- Recruitment Updates
- Career Guidance
- Exam Information
- Government Vacancy Listing

---

# Step 15

Products (Optional)

Examples:

- Premium Job Alerts
- Resume Review
- Career Consultation
- Notification Services

---

# Step 16

Messaging

Enable:

- Chat
- SMS (if available)

Assign a response owner.

---

# Step 17

Questions & Answers

Seed common FAQs such as:

- How do I apply?
- Is registration free?
- How often are jobs updated?
- Which states are covered?
- Are private jobs listed?

---

# Step 18

Reviews

Target

100+ genuine reviews

Respond to every review within 48 hours.

Never purchase fake reviews.

---

# Step 19

Google Posts

Publish weekly.

Examples:

- Latest Government Jobs
- Exam Notifications
- Admit Card Released
- Results Declared
- Career Tips
- Website Updates

---

# Step 20

Performance Insights

Monitor:

- Search Views
- Maps Views
- Calls
- Website Clicks
- Direction Requests
- Photo Views
- Search Queries

Review monthly.

---

# Step 21

SEO Optimization

Ensure consistency across:

- Website
- Search Console
- Google Analytics
- Google Tag Manager
- Bing Places
- Social Profiles

Use identical:

- Business Name
- Phone Number
- Website URL
- Email
- Logo

---

# Step 22

Maintenance

Weekly

- Respond to Reviews
- Publish a Post
- Check Messages
- Update Photos

Monthly

- Review Insights
- Update Business Info
- Add New Services
- Check Competitors

Quarterly

- Audit Profile
- Refresh Images
- Verify NAP Consistency

---

# Verification Checklist

☐ Business Created

☐ Category Selected

☐ Phone Added

☐ Website Added

☐ Service Area Added

☐ Description Written

☐ Logo Uploaded

☐ Cover Uploaded

☐ Photos Uploaded

☐ Services Added

☐ Verification Complete

☐ Reviews Enabled

☐ Messaging Enabled

☐ Google Posts Published

☐ Insights Reviewed

---

# Related Documents

docs/05-search-console.md

docs/06-analytics.md

docs/15-local-seo.md

docs/accounts/google/GOOGLE_ANALYTICS.md

docs/accounts/google/GOOGLE_TAG_MANAGER.md

docs/accounts/google/GOOGLE_SEARCH_CONSOLE.md
